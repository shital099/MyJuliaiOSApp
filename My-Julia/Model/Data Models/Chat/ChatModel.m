...
- (void)populateRandomDataSource {
    ...
    [self.dataSource addObjectsFromArray:[self additems:3]];
}
...
//    [dataDic setObject:URLStr forKey:@"strIcon"];
    ...
    [dataDic setObject:@(UUMessageFromMe) forKey:@"from"];
    [dataDic setObject:[[NSDate date] description] forKey:@"strTime"];
    [dataDic setObject:dic[@"strContent"] forKey:@"strName"];
    [dataDic setObject:[UIImage imageNamed:@"user"] forKey:@"strIcon"];

    [message setWithDict:dataDic];
    [message minuteOffSetStart:previousTime end:dataDic[@"strTime"]];
    messageFrame.showTime = message.showDateLabel;
    messageFrame.screen_size = self.screen_size;
...
- (NSArray *)additems:(NSInteger)number
{
    ...
    for (int i=0; i<number; i++) {
        ...
        [message minuteOffSetStart:previousTime end:dataDic[@"strTime"]];
        messageFrame.showTime = message.showDateLabel;
        messageFrame.showName = self.isGroupChat;
        [messageFrame setMessage:message];
