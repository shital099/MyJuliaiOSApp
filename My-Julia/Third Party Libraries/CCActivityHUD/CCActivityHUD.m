...
- (void)show {
    [self showWithType:CCActivityHUDIndicatorTypeMinorArcWithCircle];
}
...
- (void)initializeIndicatoeLayerWithType:(CCActivityHUDIndicatorType)type {
    switch (type) {
        ...
        case CCActivityHUDIndicatorTypeMinorArc:
            [self initializeMinorArc];
            break;
        case CCActivityHUDIndicatorTypeMinorArcWithCircle:
            [self initializeMinorArcWithCircle];
            break;
...
- (void)initializeMinorArcWithCircle {
    ...
    //Add Dot between circle
    CGFloat length1 = ViewFrameWidth/15;
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.indicatorCAShapeLayer.frame.size.width-length1*3, self.indicatorCAShapeLayer.frame.size.height-length1*3, length1*3, length1*3) ];
    imageView.layer.cornerRadius = imageView.frame.size.width / 2;
    imageView.backgroundColor = [UIColor whiteColor];
    [self.viewForFirstBaselineLayout addSubview:imageView];
...
- (CGRect)originalFrame {
    CGFloat length = 90; //BoundsWidthFor(Screen)/8;
    return CGRectMake(-4*length, -4*length, length, length);
}
