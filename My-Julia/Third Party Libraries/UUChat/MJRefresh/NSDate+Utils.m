...
+ (NSInteger)daysOffsetBetweenStartDate:(NSDate *)startDate endDate:(NSDate *)endDate
{
    ...
    */
    ...
    NSDateComponents *components = [gregorianCalendar components:unitFlags fromDate:startDate  toDate:endDate  options:0];
...
- (NSString *)stringYearMonthDayCompareToday:(NSInteger) days
{
    NSString *str;
    if (days == 0) {
        str = @"Today";
...
+ (NSDate *)dateFromDate:(NSDate *)date withFormat:(NSString *)format withNewFormat:(NSString *)newFormat {
    NSDateFormatter *inputFormatter = [[NSDateFormatter alloc] init];
    [inputFormatter setDateFormat:format];
    NSString *str = [inputFormatter stringFromDate:date];
    [inputFormatter setDateFormat:newFormat];
    NSDate *newDate = [inputFormatter dateFromString:str];
    return newDate;
}

+ (NSString *)timeFromDate:(NSDate *)date withFormat:(NSString *)format {
    NSDateFormatter *inputFormatter = [[NSDateFormatter alloc] init];
    [inputFormatter setDateFormat:format];
