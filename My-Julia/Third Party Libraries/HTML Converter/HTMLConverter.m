...
- (NSString *)toHTML:(NSString *)text
{
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        else if ([self string:text from:i hasPrefix:@"www."])
        {
            link = [[NSMutableString alloc] initWithString:@"www."];
            i += 4;
        }
        else if ([self string:text from:i hasPrefix:@"Www."])
        {
...
- (NSString *)cutScheme:(NSString *)url
{
    ...
    else if ([url hasPrefix:@"https://"] || [url hasPrefix:@"Https://"])
    {
        return [url substringFromIndex:8];
    }
    else if ([url hasPrefix:@"Www."] || [url hasPrefix:@"www."])
    {
        return [url substringFromIndex:8];
    }
    else
